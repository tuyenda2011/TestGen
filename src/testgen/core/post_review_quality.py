...
def _first_issue_type(execution_summary: dict[str, Any], diagnostics: dict[str, Any]) -> str:
    issue = execution_summary.get("execution_issue")
    if isinstance(issue, dict) and issue.get("type"):
        return str(issue.get("type"))
    return str(
        diagnostics.get("external_execution_issue_type")
    ...
    *,
    ...
    passed = bool(
        execution_summary.get(
            "passed",
            diagnostics.get("external_execution_passed", diagnostics.get("pytest_passed", False)),
        )
    )
    ...
    coverage = _safe_float(
        execution_summary.get(
            "coverage_percent",
            diagnostics.get("external_coverage_percent", diagnostics.get("pytest_combined_coverage_percent", 0.0)),
        )
    )
    issue_type = _normalize_e2e_issue_type(
        framework,
        _first_issue_type(execution_summary, diagnostics),
        coverage_supported,
    )
