...
- CRITICAL: For Postman, using `pm.expect(pm.response.text()).to.be.empty` to assert an empty JSON object is logically wrong and fails at runtime (because `"{{}}"` is not empty). Flag this as "Lỗi nghiêm trọng" and instruct the generator to use `pm.expect(Object.keys(pm.response.json())).to.be.empty` instead.
...
- CRITICAL: For Postman, DO NOT use `pm.expect(pm.response.text()).to.be.empty` to check for empty JSON objects like `{{}}` or `[]`. This assertion will FAIL at runtime because `"{{}}"` is not an empty string. Instead, parse the JSON and check keys: `pm.expect(Object.keys(pm.response.json())).to.be.empty`.
- CRITICAL: For Postman, NEVER generate literal excessively long strings (e.g., hundreds of characters like "AAAA...") or placeholder strings like "LONG_STRING_500_CHARS" in test payloads. For boundary tests, you MUST use Postman environment variables via Pre-request Script (e.g. `pm.environment.set('maxTitle', 'T'.repeat(500));`) and use the `{{{{maxTitle}}}}` variable in the JSON body.
- CRITICAL: For Postman, MUST write strong assertions. DO NOT just check `pm.response.to.have.status()`. You MUST check the response body schema, field existence, and data types (e.g., `pm.expect(pm.response.json().id).to.be.a('number')`).
- CRITICAL: For Postman, your assertion for the HTTP Status Code MUST STRICTLY match the Expected Result defined in the Test Plan (which is derived from the Business Requirement). DO NOT "fix" or change the expected status code to match the actual behavior of a dummy/mock API (like JSONPlaceholder returning 201 when it should return 400). If the requirement says 400, your code MUST assert 400. It is EXPECTED that the test will FAIL if the API is broken.
